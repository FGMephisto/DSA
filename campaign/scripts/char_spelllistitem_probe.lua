...
function getAttribut( probe, nummer )
	...
	local node = getDatabaseNode().getParent().getParent().getParent().getChild("attribute");
...
function onDrag(button, x, y, draginfo)					
	...
	local w = getDatabaseNode().getParent().getChild("wert");	
	if not w then
		w = "0";
	else
		w = w.getValue();
	...
	local b = getDatabaseNode().getParent().getChild("ebe");	
