...
function resetHealth(nodeChar)
	...
	-- ToDo: Check
	local nTemporary = DB.getValue(nodeChar, "hp.temporary", 0);
	nTemporary = nTemporary - (nLevel * 8);
	if nTemporary < 0 then
		nTemporary = 0;
	end
	DB.setValue(nodeChar, "hp.temporary", "number", nNonlethal);
